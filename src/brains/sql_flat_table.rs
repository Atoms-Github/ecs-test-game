...
pub struct BrainSqlFlatTable<T> {
    database: T,
    simultaneous: bool,
}

impl<T: SqlInterface> BrainSqlFlatTable<T> {
...
impl<T: SqlInterface> Brain for BrainSqlFlatTable<T> {
    ...
    fn get_entities(&mut self, universe_id: usize) -> Vec<(Point, Color)> {
        return self
            .database
            .get_entities(universe_id, "SELECT pos_x, pos_y, color_r FROM entities")
            .iter()
            .map(|e| (e.pos, e.color))
            .collect();
    }
    ...
    fn tick_system(&mut self, system: &SystemType, delta: f32, settings: &GuiSettings) {
        match system {
            ...
            SystemType::DeleteExpired => {
                self.database
                    .execute("DELETE FROM entities WHERE timed_life < 0;", vec![]);
            }
